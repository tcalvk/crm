...
class ContractsDB {
    public function get_evergreen_15() {
        ...
        ct.Zip "BillingZip", ct.Email "BillingEmail", ctt.BaseAmt + ifnull(c.CAM,0) "Total", u.email "ContractOwnerEmail"
        from Contract c 
    ...
    public function get_fixed_1() {
        ...
        $query =  'select c.ContractId, p.PropertyId, c.BaseAmt, c.CAM, c.DueDate, cp.Name "CompanyName", cp.Address1 "CompanyAddress1",
                  ...
                  "CompanyState", cp.Zip "CompanyZip", ct.Name "BillingName", ct.Attention, ct.Address1 "BillingAddress1", 
                  ct.Address2 "BillingAddress2", ct.Address3 "BillingAddress3", ct.City "BillingCity", ct.StateId "BillingState", 
                  ct.Zip "BillingZip", ct.Email "BillingEmail", c.BaseAmt + ifnull(c.CAM,0) "Total", p.Name "PropertyName", c.NumPaymentsDue, 
    ...
    public function create_contract($data) {
        ...
        $statement->bindValue(':PropertyId', $data['PropertyId'], $data['PropertyId'] === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
        $statement->bindValue(':CustomerId', $data['CustomerId'], PDO::PARAM_INT);
        $statement->bindValue(':CompanyId', $data['CompanyId'], $data['CompanyId'] === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
        $statement->bindValue(':BaseAmt', $data['BaseAmt'], $data['BaseAmt'] === null ? PDO::PARAM_NULL : PDO::PARAM_STR);
        $statement->bindValue(':CAM', $data['CAM'], $data['CAM'] === null ? PDO::PARAM_NULL : PDO::PARAM_STR);
        ...
        $statement->bindValue(':LateDate', $data['LateDate'], $data['LateDate'] === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
        $statement->bindValue(':LateFee', $data['LateFee'], $data['LateFee'] === null ? PDO::PARAM_NULL : PDO::PARAM_STR);
        $statement->bindValue(':StatementSendDate', $data['StatementSendDate'], PDO::PARAM_INT);
        $statement->bindValue(':NumPaymentsDue', $data['NumPaymentsDue'], $data['NumPaymentsDue'] === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
        $statement->bindValue(':TotalPaymentsDue', $data['TotalPaymentsDue'], $data['TotalPaymentsDue'] === null ? PDO::PARAM_NULL : PDO::PARAM_INT);
        $statement->bindValue(':ContractType', $data['ContractType']);
        $statement->bindValue(':TestContract', (int) $data['TestContract'], PDO::PARAM_INT);
